...
class TestYInstance extends Doc {
  ...
  _receive (message, remoteClient) {
    map__namespace.setIfUndefined(this.receiving, remoteClient, () => []).push(message);
  }
}
...
class ProsemirrorBinding {
  ...
  _forceRerender () {
    ...
    this.mux(() => {
      ...
      const fragment = PModel__namespace.Fragment.from(fragmentContent);
      const tr = this._tr;
      ...
      // @ts-ignore
      tr.replace(0, this.prosemirrorView.state.doc.content.size, new PModel__namespace.Slice(fragment, 0, 0));
...
const testAddToHistory = tc => {
  ...
  view.dispatch(view.state.tr.insert(0, /** @type {any} */ (schema.node('paragraph', undefined, schema.text('123')))));
  const yxml = ydoc.get('prosemirror');
  t__namespace.assert(yxml.length === 2 && yxml.get(0).length === 1, 'contains inserted content');
  undo(view.state);
  ...
  t__namespace.assert(yxml.length === 2 && yxml.get(0).length === 1, 'contains inserted content');
  undo(view.state);
  t__namespace.assert(yxml.length === 2 && yxml.get(0).length === 1, 'insertion was *not* undone');
};
...
const testAddToHistoryIgnore = tc => {
  ...
  const yxml = ydoc.get('prosemirror');
  t__namespace.assert(yxml.length === 3 && yxml.get(0).length === 1, 'contains inserted content (1)');
  view.dispatch(view.state.tr.insert(0, /** @type {any} */ (schema.node('paragraph', undefined, schema.text('abc')))).setMeta('addToHistory', false));
  t__namespace.assert(yxml.length === 4 && yxml.get(0).length === 1, 'contains inserted content (2)');
  view.dispatch(view.state.tr.insert(0, /** @type {any} */ (schema.node('paragraph', undefined, schema.text('xyz')))));
  t__namespace.assert(yxml.length === 5 && yxml.get(0).length === 1, 'contains inserted content (3)');
  undo(view.state);
  t__namespace.assert(yxml.length === 4, 'insertion (3) was undone');
  undo(view.state);
  console.log(yxml.toString());
  t__namespace.assert(yxml.length === 1 && yxml.get(0).toString() === '<paragraph>abc</paragraph>', 'insertion (1) was undone');
};
