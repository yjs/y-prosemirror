...
export const $syncPluginState = s.$object({
  ...
  /**
   * Predicate deciding whether the current ProseMirror document is the
   * integrator's initial (empty) state. `null` keeps the schema-default
   * fingerprint check. See {@link IsInitialContent} and {@link syncPlugin}.
   */
  isInitialContent: /** @type {s.Schema<IsInitialContent>} */ (s.$function).nullable,
  /**
...
export const $syncPluginStateUpdate = s.$object({
  ...
  isInitialContent: /** @type {s.Schema<IsInitialContent>} */ (s.$function).nullable.optional,
    ...
    state: {
      init: () => {
        return $syncPluginState.expect({
          ...
          isInitialContent: opts.isInitialContent || null,
          binding: null
        })
      },
