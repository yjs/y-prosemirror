...
} from "../src/y-stitchkit.js"
import { EditorState, Plugin, TextSelection } from "@stitchkit/pm/state"
import { EditorView } from "@stitchkit/pm/view"
import { Schema } from "@stitchkit/pm/model"
import * as basicSchema from "@stitchkit/pm/schema-basic"
import { findWrapping } from "@stitchkit/pm/transform"
...
export const testPluginIntegrity = (_tc) => {
  ...
  const view = new EditorView(null, {
    ...
    state: EditorState.create({
      schema,
      plugins: [ySyncPlugin(ydoc.get("prosemirror", Y.XmlFragment)), yUndoPlugin(), customPlugin],
    }),
  })
  view.dispatch(
    view.state.tr.insert(0, /** @type {any} */ (schema.node("paragraph", undefined, schema.text("hello world"))))
  )
  t.compare(
...
export const testChangeOrigin = (_tc) => {
  ...
  view.dispatch(view.state.tr.insert(0, /** @type {any} */ (schema.node("paragraph", undefined, schema.text("world")))))
...
export const testInsertDuplication = (_tc) => {
  ...
  view1.dispatch(view1.state.tr.insert(0, /** @type {any} */ (schema.node("paragraph"))))
  const sync = () => {
...
export const testAddToHistory = (_tc) => {
  ...
  view.dispatch(
    view.state.tr
      .insert(0, /** @type {any} */ (schema.node("paragraph", undefined, schema.text("123"))))
...
export const testAddToHistoryIgnore = (_tc) => {
  ...
  view.dispatch(
    view.state.tr
      .insert(0, /** @type {any} */ (schema.node("paragraph", undefined, schema.text("abc"))))
      .setMeta("addToHistory", false)
  )
...
const pmChanges = [
  ...
  (_y, gen, p) => {
    // format text
    const insertPos = prng.int32(gen, 0, p.state.doc.content.size)
    const formatLen = math.min(prng.int32(gen, 0, p.state.doc.content.size - insertPos), 2)
