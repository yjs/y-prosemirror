...
const firstChildName = (d) => {
  for (const op of d.children) {
    if (delta.$insertOp.check(op)) {
      for (const it of op.insert) if (delta.$deltaAny.check(it)) return it.name
    }
  }
  return null
}
 ...
 * merges as usual.
 *
 * @param {import('lib0/delta').DeltaAny} a removed (old) node
 * @param {import('lib0/delta').DeltaAny} b inserted (new) node
 * @returns {boolean} whether `a` and `b` are the same node (diff in place) vs different (replace)
 */
export const blockMatchNodes = (a, b) =>
